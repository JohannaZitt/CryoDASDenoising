...
def scale_array(arr):
    # Formel: (x * 2) - 1
    # Dies wandelt Werte von [0,1] in [-1,1] um
    return (arr * 2) - 1
...
experiments = ["01_ablation_horizontal", "03_accumulation_horizontal", "07_retrained_combined200", "11_vanende", "12_vanende_finetuned_cryo", "13_isken_filter", "14_julius_filter", "15_DASDL" ]
...
for experiment in experiments:
    ...
    for i, id in enumerate(ids):
        ...
        verb = 1
        rect = [3, 3, 1]
        local_similarity = pyortho.localsimi(denoised_data_norm, raw_minus_denoised_data_norm, rect, niter, eps, verb)
        #local_similarity_norm = scale_array(local_similarity)
        ...
        print("Max LS: ", np.max(local_similarity))
        print("Min LS: ", np.min(local_similarity))
        print("Max RAW: ", np.max(raw_data_norm))
        print("Min Raw: ", np.min(raw_data_norm))
        ...
        """ Plotting Raw - Denoised: """
        axs[i, 0].imshow(raw_minus_denoised_data_norm, cmap=cmap, aspect="auto", interpolation="antialiased",
        ...
        """ Plotting Local Similarity: """
        im = axs[i, 1].imshow(local_similarity, cmap=cmap, aspect="auto", interpolation="antialiased",
                  extent=(0 ,(t_end_das-t_start_das)/400,0,ch_end * ch_ch_spacing/1000),
    ...
    """ Set Titles """
    axs[0, 0].set_title("Noisy-Denoised", y=1.0, fontsize=fs+2) #Removed Noise (better?!)
    axs[0, 1].set_title("Local Similarity", y=1.0, fontsize=fs+2)
    ...
    axs[0, 2].set_xticks([1, 3, 5, 7])
    axs[0, 2].set_xticklabels([])
    axs[1, 2].set_xticks([1, 3, 5, 7])
    axs[1, 2].set_xticklabels([])
    axs[2, 2].set_xticks([1, 3, 5, 7], [1, 3, 5, 7], fontsize=fs-2)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig("plots/comparison/imshow_" + experiment + "_noise_ls_3x3.pdf", dpi=400)
